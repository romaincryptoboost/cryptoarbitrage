/*
  # Schema complet pour Crypto-Arbitrage

  1. Types personnalisés
    - Rôles utilisateur (CLIENT, ADMIN)
    - Statuts utilisateur (ACTIVE, SUSPENDED)
    - Types de crypto-monnaies supportées
    - Types de transactions et leurs statuts

  2. Tables principales
    - `users` - Profils utilisateurs
    - `plans` - Plans d'investissement
    - `wallets` - Portefeuilles crypto
    - `transactions` - Historique des transactions
    - `subscriptions` - Souscriptions aux plans
    - `notifications` - Système de notifications
    - `rates_cache` - Cache des taux crypto
...
DROP TYPE IF EXISTS crypto_asset CASCADE;
